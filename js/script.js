...
document.addEventListener("DOMContentLoaded", () => {
    ...
    videos.forEach(video => {
        video.setAttribute("poster", "/wasm/images/video/poster/default.svg")
    });
    ...
    const remapArticles = (data) => {
        return data.map((item) => {
            return { id: item[0], title: item[1], description: item[2], url: `/wasm/article/${kebabCase(item[1])}`, open_graph: item[3] };
        });
    };
    ...
    if (articlesContainer) {
        fetch("/wasm/js/articles.json").then(res => res.json()).then(data => {
    ...
    function renderList(items, container, paginatorId, page, isSearch, onPageChange) {
        ...
        pageItems.forEach(item => {
            ...
            if (item.open_graph === true) {
                ...
                imageTop.style.backgroundImage = `url(/wasm/images/open-graph/article/${item.id}.webp)`;
                ...
                imageBottom.src = `/wasm/images/open-graph/article/${item.id}.webp`;
